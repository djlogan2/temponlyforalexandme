...
import { BookEntry, BookRecord } from "/lib/records/BookRecord";
...
export default class BookService extends Stoppable {
  private static BOOKNAME = "defaultbook";

  // BookCollection = new Mongo.Collection("book");

  private dao: WritableBookDao;

  private log: ServerLogger;

  public findBook(fen: string): BookRecord | undefined {
    const hash = this.getHash(fen).toString();
    return this.dao.readOne({ key: hash });
  }

  private loadBookFile() {
    const content = Buffer.from(Assets.getBinary("book.bin"));
    let record: BookRecord | undefined;
    // let entry;
    let idx = 0;
    do {
      ...
      const bookentry = this.convertMove(fileentry);
      if (!!record && record.key !== key) {
        this.dao.insert({ ...record });
        record.key = key;
        record.entries = [bookentry];
      } else if (!record) {
        record = {
          name: BookService.BOOKNAME,
          key,
          entries: [bookentry],
        };
      } else record.entries.push(bookentry);
    } while (idx < content.length);
    if (record) this.dao.insert(record);
  }

  constructor(parent: Stoppable | null, dao: WritableBookDao) {
    super(parent);
    this.dao = dao;
    this.log = new ServerLogger(this, "BookService_ts");
    Meteor.startup(() => {
      if (!Meteor.isTest && !Meteor.isAppTest && this.dao.count({}) === 0) {
        console.log(
          "We are going to do an initial load of the book collection. This will take a REALLY REALLY REALLY long time. Do not kill me. Let me run.",
        );
        this.loadBookFile();
  ...
  private convertMove(entry: FileEntry): BookEntry {
    const toFile = entry.move & 7;
    const toRow = (entry.move >>> 3) & 7;
    const fromFile = (entry.move >>> 6) & 7;
    const fromRow = (entry.move >>> 9) & 7;
    const promote = (entry.move >>> 12) & 7;
    ...
    const fromSquare: SquareType = {
      row: fromRow,
      file: fromFile,
      text: "abcdefgh"[fromFile] + (fromRow + 1),
    };
    let toSquare: SquareType = {
      row: toRow,
      file: toFile,
      text: "abcdefgh"[toFile] + (toRow + 1),
    };
    //
    ...
    return {
      weight: entry.weight,
      learn: entry.learn,
      smith: fromSquare.text + toSquare.text + promotePiece,
    };
  }

  private getHash(fen: string): bigint {
    const chess = new Chess(fen);
    const files = "abcdefgh";
    let hash = BigInt(0);
    for (let rank = 0; rank < 8; rank += 1) {
      for (let file = 0; file < 8; file += 1) {
        const piece = chess.get((files[file] + (rank + 1)) as Square);
    ...
    if (castling.indexOf("K") !== -1)
      hash ^= BookService.RANDOM64[BookService.RANDOMCASTLE];
    if (castling.indexOf("Q") !== -1)
      hash ^= BookService.RANDOM64[BookService.RANDOMCASTLE + 1];
    if (castling.indexOf("k") !== -1)
      hash ^= BookService.RANDOM64[BookService.RANDOMCASTLE + 2];
    if (castling.indexOf("q") !== -1)
      hash ^= BookService.RANDOM64[BookService.RANDOMCASTLE + 3];
    ...
    if (epsquare !== "-") {
      if (
        chess
          .moves({ verbose: true })
          .some((move) => move.flags.indexOf("e") !== -1)
      ) {
        const epfile = "abcdefgh".indexOf(epsquare[0]);
        if (epfile !== -1) {
          hash ^= BookService.RANDOM64[BookService.RANDOMENPASSANT + epfile];
        }
      }
    }

    if (chess.turn() === "w")
      hash ^= BookService.RANDOM64[BookService.RANDOMTURN];

    return hash;
  }

  private getPieceHash(rank: number, file: number, piece: Piece): bigint {
    let v: number = 0;
    switch (piece.type) {
      case "p":
        v = 0;
    ...
    if (piece.color === "w") v += 1;
    v = 64 * v + 8 * rank + file;
    return BookService.RANDOM64[v];
  }
  ...
  private static RANDOMCASTLE = 768;

  private static RANDOMENPASSANT = 772;

  private static RANDOMTURN = 780;

  private static RANDOM64 = [

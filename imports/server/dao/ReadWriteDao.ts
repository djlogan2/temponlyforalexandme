...
export default class ReadWriteDao<T> extends ReadOnlyDao<T> {
  private readonly boundinsert: (record: Mongo.OptionalId<T>) => string;

  private readonly boundremove: (selector: Mongo.Selector<T>) => number;

  private readonly boundupdate: (
    selector: Mongo.Selector<T>,
    modifier: Mongo.Modifier<T>,
  ) => number;

  private readonly boundupsert: (
    selector: Mongo.Selector<T>,
    modifier: Mongo.Modifier<T>,
  ...
  constructor(collection: CollectionNames, parent: Stoppable | null) {
    ...
    this.boundinsert = Meteor.bindEnvironment((record: Mongo.OptionalId<T>) =>
      this.mongocollection.insert(record),
    );
    this.boundremove = Meteor.bindEnvironment((selector: Mongo.Selector<T>) =>
      this.mongocollection.remove(selector),
    );
    this.boundupdate = Meteor.bindEnvironment(
      (selector: Mongo.Selector<T>, modifier: Mongo.Modifier<T>) =>
        this.mongocollection.update(selector, modifier),
    );
    this.boundupsert = Meteor.bindEnvironment(
      (selector: Mongo.Selector<T>, modifier: Mongo.Modifier<T>) =>
        this.mongocollection.upsert(selector, modifier),
    );
  }

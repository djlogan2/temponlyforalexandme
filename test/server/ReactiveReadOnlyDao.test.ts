  ...
  switch (what) {
    case "add":
      return new Promise<void>((resolve, reject) => {
        global.ICCServer.collections.reactivereadonlydaotest.insert(
          { _id: "found1", data: "data1" },
          (err: any) => {
            if (err) reject(err);
            else resolve();
          },
    ...
    case "change":
      return new Promise<void>((resolve, reject) => {
        global.ICCServer.collections.reactivereadonlydaotest.update(
          { _id: "found1" },
          { $set: { data: "data3" } },
          undefined,
          (err: any) => {
    ...
    case "remove":
      return new Promise<void>((resolve, reject) => {
        global.ICCServer.collections.reactivereadonlydaotest.remove(
          { _id: "found1" },
          (err: any) => {
            if (err) reject(err);
            else resolve();
...
if (!global.ICCServer.collections.readonlydaotest)
  global.ICCServer.collections.reactivereadonlydaotest =
    new Mongo.Collection<TestRecord>("reactivereadonlydaotest");

Meteor.publish("reactivereadonlydaotest", () =>
  global.ICCServer.collections.reactivereadonlydaotest.find(),
);

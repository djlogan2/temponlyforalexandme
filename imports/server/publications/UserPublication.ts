...
export default class UserPublication extends Publication<UserRecord> {
  ...
  constructor(
    ...
    this.logger2.trace(() => `${connection?._id} New UserPublication`);
    ...
    connection.events.on("userlogout", () => {
      this.logger2.trace(() => `${connection._id} event::userlogout`);
      this.unsetUser();
    });
    if (connection.user) {
      this.logger2.trace(
  ...
  private setUser(id: string): void {
    this.logger2.trace(() => `${this.connectionid} setUser id=${id}`);
    this.setSelector({ _id: id }, "include", STANDARD_MEMBER_FIELDS);
  }

  private unsetUser(): void {
    this.logger2.trace(() => `${this.connectionid} unsetUser`);
    this.killCursor();
  }
}

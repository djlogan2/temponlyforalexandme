...
export default class SubscriptionService extends Stoppable {
  ...
  public getSubscriptionEventEmitter(publication: string): ICCEventEmitter {
    if (!global.ICCServer.client)
      global.ICCServer.client = { dao: {}, subscriptions: {} };
    if (!global.ICCServer.client.subscriptions[publication])
      global.ICCServer.client.subscriptions[publication] =
        new SubscriptionEventEmitter(publication, this);
    return global.ICCServer.client.subscriptions[publication].newEmitter();
  }

  protected stopping(): void {
    if (!global?.ICCServer?.client?.subscriptions) return;
    Object.values(global.ICCServer.client.subscriptions).forEach((sub) =>
      sub.stop(),
    );

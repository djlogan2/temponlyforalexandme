const P1WINCHANCE = 0.20;
const P1DRAWCHANCE = 0.10;
...
function winDrawLossAssessValues(robject1, robject2) {
  ...
  const resultw =
    robject1.rating +
    ((32 + adjust) * Kopp * KYou * (1 - 1 / (1 + 10 ** ((robject2.rating - robject1.rating) / 400.0)))) /
      KYourDiv /
      KOppdiv;
  const resultd =
    robject1.rating +
  ...
  const resultl =
    robject1.rating +
    ((32 + adjust) * Kopp * KYou * (0 - 1 / (1 + 10 ** ((robject2.rating - robject1.rating) / 400.0)))) /
      KYourDiv /
      KOppdiv;
...
for(let x = 0 ; x < GAMECOUNT ; x += 1) {
  ...
  console.log(`p1 rating=${  p1.rating  } p2 rating=${  p2.rating  } weight=${color.weight} random=${color.random} chance=${color.chance} color=${color.color} result=${  stringresult} avgrating=${(p1.rating+p2.rating)/2}`);
}
const whitecount = games.reduce((count, color) => count + (color === "w" ? 1 : 0), 0);
const blackcound = games.length - whitecount;
console.log(`White=${whitecount} Black=${blackcound} WhiteRatio=${whitecount / games.length} BlackRatio=${blackcound / games.length}`);

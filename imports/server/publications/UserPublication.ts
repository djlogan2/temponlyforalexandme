...
export default class UserPublication extends Publication<UserRecord> {
  ...
  constructor(
    ...
    this.logger = new ServerLogger(this, "UserPublication_js");
    this.logger.debug(() => `${connection?._id} New UserPublication`);
    ...
    connection.events.on("userlogout", () => {
      this.logger.debug(() => `${connection._id} event::userlogout`);
      this.unsetUser();
    });
    if (connection.user) {
      this.logger.debug(
  ...
  private setUser(id: string): void {
    this.logger.debug(() => `${this.connectionid} setUser id=${id}`);
    this.setSelector({ _id: id }, "include", STANDARD_MEMBER_FIELDS);
  }

  private unsetUser(): void {
    this.logger.debug(() => `${this.connectionid} unsetUser`);
    this.killCursor();
  }
}
